...
add_to_log($course->id, 'educationplusplus', 'editPES', "editPES.php?id={$cm->id}", $educationplusplus->name, $cm->id);

/// Print the page header

$PAGE->set_url('/mod/educationplusplus/editPES.php', array('id' => $cm->id));
...
echo "<link rel='stylesheet' type='text/css' href='./css/usecaseboxes.css'>
	<div class='floatingdiv'>Use Case Scenario(s): 5.3.4, 5.3.3</div>";
...
echo '<div id="introbox" style="width:900px;margin:0 auto;text-align:center;margin-bottom:15px;">
		...
		<p>Edit the selected Point Earning Scenario Below.</p>
	  </div>';
			...
			<div id="form" style="width:400px;height:600px;overflow:auto;">
				<form id="pesform" name="pesform" method="post" onsubmit="return validate()" action="persistUpdatedPES.php?id='. $cm->id .'&pes=' . $idOfPesToEdit . '" name="pes-creator" id="pes-creator" style="padding-left:10px;padding-right:10px;">
					...
					<table>
						...
						<tr>
							<td>Expiry Date</td>
							<td><input type="text" value="' . $PES->expiryDate->format('Y-m-d') . '" style="margin-right:10px;width:200px;" id="pesExpiryDate" name="pesExpiryDate"><br/><span id="expReq" style="color:red;display:none;">You Must Specify an Expiry Date</span></td>
						</tr>
						<tr>
